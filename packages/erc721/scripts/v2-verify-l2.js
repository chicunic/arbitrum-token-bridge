...
async function main() {
  ...
  const l2ERC721GatewayLogic = await ethers.getContractAt(
    'L2ERC721Gateway',
    l1Network.tokenBridge.l1CustomGatewayLogic,
  );
  try {
    await run('verify:verify', {
      address: l2ERC721GatewayLogic.address,
      constructorArguments: [],
    });
  } catch (err) {
    console.error(err.message);
  }
  console.log('2-1: L2 ERC721Gateway Logic verified at', l2ERC721GatewayLogic.address);

  const l2ERC721GatewayProxy = await ethers.getContractAt(
    '@openzeppelin/contracts-0.8/proxy/transparent/TransparentUpgradeableProxy.sol:TransparentUpgradeableProxy',
    l1Network.tokenBridge.l1CustomGateway,
  );
  try {
    await run('verify:verify', {
      address: l2ERC721GatewayProxy.address,
      constructorArguments: [l2ERC721GatewayLogic.address, l1Network.tokenBridge.l2ProxyAdmin, '0x'],
    });
  } catch (err) {
    console.error(err.message);
  }
  console.log('2-2: L1 ERC721Gateway Proxy verified at', l2ERC721GatewayProxy.address);
}
