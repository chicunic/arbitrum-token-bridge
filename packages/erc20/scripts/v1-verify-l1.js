...
async function main() {
  ...
  const l1CustomGatewayLogic = await ethers.getContractAt(
    'L1CustomGateway',
    l1Network.tokenBridge.l1CustomGatewayLogic,
  );
  try {
    await run('verify:verify', {
      address: l1CustomGatewayLogic.address,
      constructorArguments: [],
    });
  } catch (err) {
    console.error(err.message);
  }
  console.log('2-1: L1 CustomGateway Logic verified at', l1CustomGatewayLogic.address);

  const l1CustomGatewayProxy = await ethers.getContractAt(
    '@openzeppelin/contracts-0.8/proxy/transparent/TransparentUpgradeableProxy.sol:TransparentUpgradeableProxy',
    l1Network.tokenBridge.l1CustomGateway,
  );
  try {
    await run('verify:verify', {
      address: l1CustomGatewayProxy.address,
      constructorArguments: [l1CustomGatewayLogic.address, l1Network.tokenBridge.l1ProxyAdmin, '0x'],
    });
  } catch (err) {
    console.error(err.message);
  }
  console.log('2-2: L1 CustomGateway Proxy verified at', l1CustomGatewayProxy.address);
}
